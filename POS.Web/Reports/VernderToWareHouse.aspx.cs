...
{
    public partial class VernderToWareHouse : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                ReportViewer1.LocalReport.ReportPath = Server.MapPath("VenrnderToWareHouseReports.rdlc");
                ...
                int VenderId = Convert.ToInt32(Request.QueryString["vendorId"].ToString());
                ...
                List<VendorWhereHouseViewModel> list = ReportServices.GetVenderToWarHouseReport(fromDate, toDate , ItemId, VenderId).ToList();
                var user = Session[WebUtil.CURRENT_USER] as UserViewModel;
                foreach (var item in list)
                {
                    item.Remarks = user.UserName;
                }
